...
`Windows 11` `Windows Server` `Active Directory` `DNS`  
`Group Policy` `User & Group Management` `File Permissions` `PowerShell`

### ☁️ Microsoft 365 & Identity

`Microsoft 365` `Microsoft Entra ID` `MFA`  
`Identity & Access Management` `User Administration` `Authentication`

### 🌐 Networking

`TCP/IP` `IPv4` `Subnetting` `VLANs` `DHCP`  
`DNS` `Routing` `Switching` `NAT` `ACLs` `ICMP`
...
`ServiceNow` `Jira` `osTicket` `Cisco Packet Tracer`  
`VirtualBox` `PowerShell` `Git` `GitHub`
